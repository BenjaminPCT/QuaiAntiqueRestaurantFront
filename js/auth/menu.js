...
function fetchMenus() {
    const apiUrl = "http://127.0.0.1:8000/api/menus"; // Remplace par l'URL de notre API
...
function displayMenus(menus) {
    ...
    menus.forEach(menu => {
        ...
        titleElement.innerHTML = `${sanitizeHtml(menu.title)} <br>${sanitizeHtml(menu.price)}€</br>`;

        const entriesElement = document.createElement('p');
        entriesElement.innerHTML = `Entrées <br/><span class="centered">${sanitizeHtml(menu.entries)}</span><br/><br/>`;

        const dishesElement = document.createElement('p');
        dishesElement.innerHTML = `Plats<br/><span class="centered">${sanitizeHtml(menu.dishes)}</span><br/><br/>`;

        const dessertsElement = document.createElement('p');
        dessertsElement.innerHTML = `Desserts <br/><span class="centered">${sanitizeHtml(menu.desserts)}</span><br/><br/>`;
